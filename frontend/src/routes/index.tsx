    ...
    <div className={pageClass}>
      <div className={heroClass}>
        ...
        <p className={subtitleClass}>
          Log editions by UPC, attach copies with condition notes, and refresh fair
          market values from PriceCharting when your API token is set.
        </p>
        <div className={ctaRowClass}>
          <Link to="/inventory" className={buttonVariants({ variant: 'primary', size: 'md' })}>
            Open inventory
          </Link>
          <Link to="/inventory/add" className={buttonVariants({ variant: 'secondary', size: 'md' })}>
            Add a game
          </Link>
        </div>
      </div>
      ...
      <Card>
        <div className={cardBody}>
          ...
          <ul className={setupListClass}>
            {[
              <>Start Postgres, set <code className={codeClass}>DATABASE_URL</code>, then run <code className={codeClass}>pnpm db:migrate</code>.</>,
              <>Run <code className={codeClass}>pnpm dev:api</code> (port 4000) and <code className={codeClass}>pnpm dev</code> (port 3000). Point <code className={codeClass}>VITE_API_URL</code> at the API.</>,
              <>Optional: set <code className={codeClass}>PRICECHARTING_API_TOKEN</code> on the backend to enable market refresh.</>,
            ].map((item, i) => (
